  ...
  <el-table class="table-custom" :data="data" border style="width: 90%">
    ...
    <el-table-column v-if="type === 'edit'" label="完成投资（万元）" header-align="center">
      ...
      <el-table-column label="总投资" width="100">
        <template slot-scope="scope">
          <span> {{ formatMoney((form[`tmpKey-gov-first-${scope.$index}`] || 0) + (form[`tmpKey-drive-first-${scope.$index}`] || 0)) }} </span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    calcRateTotal(scope) {
      ...
      const plantTotal = (Number(planFirstDrive) + Number(planFirstGov) + Number(planSecondDrive) + Number(planSecondGov)) || 1;
    ...
    calcRateCurrentYear(scope) {
      ...
      const plantTotal = isFirstYear ? (Number(planFirstDrive) + Number(planFirstGov)) || 1 : (Number(planSecondDrive) + Number(planSecondGov)) || 1;
      const currentTotal = Number(form[`tmpKey-gov-first-${scope.$index}`] || 0) + Number(form[`tmpKey-drive-first-${scope.$index}`] || 0);
      const result = ((currentTotal / plantTotal * 100) || 0).toFixed(1);
