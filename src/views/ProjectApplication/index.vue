    ...
    >
      <template v-slot:search>
        <div class="inline-flex mb-6 pl-0">
          ...
          <div class="search-item">
            ...
            <el-select v-model="query.projectType" placeholder="请选择项目类型" clearable>
          ...
          <div class="search-item">
            ...
            <el-select v-model="query.projectStatus" placeholder="请选择状态" clearable>
      ...
      <template v-slot:table>
        ...
        <el-table-column label="状态" prop="projectStatus">
          <template slot-scope="scope">{{ projectStatusMap[scope.row.projectStatus] }}</template>
        </el-table-column>
      </template>

      <template v-slot:tableAction="scope">
        <el-link type="primary" @click="goDetail(scope.data.id, scope.data.projectStatus)"> 详情 </el-link>
        <div class="inline" v-if="actionControl('修改', scope.data.projectStatus)">
          <el-divider direction="vertical"></el-divider>
          <el-link type="primary" @click="goModify(scope.data.id)"> 修改 </el-link>
        </div>
        <div class="inline" v-if="actionControl('删除', scope.data.projectStatus)">
          <el-divider direction="vertical"></el-divider>
          <el-link type="primary" @click="goDelete(scope.data.id)"> 删除 </el-link>
        </div>
        <div v-if="actionControl('审核详情', scope.data.projectStatus)" class="inline">
          <el-divider direction="vertical"></el-divider>
          <el-link type="primary" @click="toAuditDetail(scope)">审核详情</el-link>
        </div>
        <div class="inline" v-if="actionControl('审核', scope.data.projectStatus)" v-permission="50002">
          <el-divider direction="vertical"></el-divider>
          <el-link type="primary" @click="verify(scope)"> 审核 </el-link>
        </div>
      </template>
    </Crud>
    <el-dialog title="审核" :visible.sync="dialogVisible" width="600px" :before-close="handleClose">
...
import { getProjectList, verifyProject, deleteProject, pogressExport } from '@/api/projectDeclare';
...
export default {
  ...
  methods: {
    ...
    verify(scope) {
      this.tips =
        this.userInfo.roleId === 1 ? '是否通过该项目审核？' : '是否通过该项目审核，若审核通过则项目提交至省级。';
      this.textarea = '';
    ...
    clickExport() {
      ...
      if (this.query.years) {
        fileName = this.query.years + '年度项目清单' + time;
      } else {
        fileName = '项目清单' + time;
      }
